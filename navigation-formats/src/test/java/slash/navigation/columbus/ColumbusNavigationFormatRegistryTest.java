...
public class ColumbusNavigationFormatRegistryTest {
    ...
    public void testGetReadFormatsSortedByExtensionCsv() {
        ...
        assertEquals(GarbleColumbusGpsType1Format.class, formats.get(3).getClass());
    }

    @Test
    public void testGetReadFormatsSortedByExtensionGps() {
        List<NavigationFormat<BaseRoute<?, ?>>> formats = registry.getReadFormatsPreferredByExtension(".gps");
        assertEquals(ColumbusGpsBinaryFormat.class, formats.get(0).getClass());
    }

    @Test
    public void testGetReadFormatsSortedByExtensionGpx() {
        List<NavigationFormat<BaseRoute<?, ?>>> formats = registry.getReadFormatsPreferredByExtension(".gpx");
        assertEquals(Gpx11Format.class, formats.get(0).getClass());
    }

    @Test
    public void testGetReadFormatsSortedByExtensionKml() {
        List<NavigationFormat<BaseRoute<?, ?>>> formats = registry.getReadFormatsPreferredByExtension(".kml");
        assertEquals(Kml22Format.class, formats.get(0).getClass());
    }

    private boolean containsFormat(List<? extends NavigationFormat<?>> formats, Class clazz) {
    ...
    public void testNoCompetitorFormats() {
        List<NavigationFormat<BaseRoute<?, ?>>> formats = registry.getReadFormats();
